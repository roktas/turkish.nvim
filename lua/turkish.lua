...
Turkish.use_mappings = function()
	if vim.o.langmap == "" then
		vim.o.langmap = [[ğ[Ğ{ü]Ü}]]
	end

	-- Normal mode <ş>: Replace current word under cursor
	if vim.fn.mapcheck("ş", "n") == "" then
		vim.api.nvim_set_keymap("n", "ş", [[:%s/<C-R>=expand("<cword>")<CR>//gce<Left><Left><Left><Left>]], {})
	end
	-- Visual mode <ş>: Replace selected text
	if vim.fn.mapcheck("ş", "v") == "" then
		-- FIXME: Avoid overwriting register S (in a perfect world)
		vim.api.nvim_set_keymap("v", "ş", [["Sy:%s/<C-R>S//gce<Left><Left><Left><Left>]], {})
	end
end
